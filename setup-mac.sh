...
# Read the first value for a key from an environment file.
read_env_value() {
    local env_file=$1
    local key=$2
...
configure_environment() {
    ...
    local default_ha_url="http://$HA_HOST:8123"
    if [[ -f ".env" ]]; then
        local existing_ha_url
        existing_ha_url=$(read_env_value ".env" HA_URL)
...
PY
    ...
    local ha_mcp_url_value
    ha_mcp_url_value=$(read_env_value ".env" HA_MCP_URL)
