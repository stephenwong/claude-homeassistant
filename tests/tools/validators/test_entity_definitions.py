...
def extractor_factory(tmp_path):
    ...
    def factory(warnings=None, info=None):
        return EntityDefinitionExtractor(
            tmp_path,
            storage,
...
def test_extracts_builtin_entities(extractor):
    entities = extractor.get_config_defined_entities()
    assert "sun.sun" in entities
    assert "zone.home" in entities


def test_template_sensor_with_null_sensors_does_not_crash(tmp_path, extractor):
...
def test_extracts_timer_counter_schedule_helpers(tmp_path, extractor):
...
def test_extracts_single_dict_template_form(tmp_path, extractor):
...
def test_extracts_group_entity(tmp_path, extractor):
    (tmp_path / "configuration.yaml").write_text(
        "group:\n  my_group:\n    entities: []\n"
    )
    entities = extractor.get_config_defined_entities()
...
def test_extracts_input_helpers(tmp_path, extractor):
    (tmp_path / "configuration.yaml").write_text(
        "input_boolean:\n  test_switch:\n    name: Test\n"
    )
    entities = extractor.get_config_defined_entities()
...
def test_extracts_template_list_entities(tmp_path, extractor):
    (tmp_path / "configuration.yaml").write_text(
        "template:\n  - sensor:\n      - name: Anyone Home\n        state: 'on'\n"
    )
    entities = extractor.get_config_defined_entities()
...
def test_extracts_template_dict_entities(tmp_path, extractor):
    (tmp_path / "configuration.yaml").write_text(
        "template:\n  sensor:\n    - name: Dict Template\n      state: 'on'\n"
    )
    entities = extractor.get_config_defined_entities()
...
def test_extracts_platform_template_sensor(tmp_path, extractor):
    (tmp_path / "configuration.yaml").write_text(
        "sensor:\n"
        "  - platform: template\n"
        "    sensors:\n"
...
def test_extracts_automation_entity_from_alias(tmp_path, extractor):
...
def test_automation_id_fallback(tmp_path, extractor):
    (tmp_path / "automations.yaml").write_text(
        "- id: backup_lights\n"
        "  trigger:\n    platform: time\n"
        "  action:\n    service: test\n"
...
def test_automation_empty_alias_and_id_skipped(tmp_path, extractor):
...
def test_extracts_script_entity(tmp_path, extractor):
...
def test_script_invalid_object_id_skipped(tmp_path, extractor):
    (tmp_path / "scripts.yaml").write_text(
        "UPPERCASE_SCRIPT:\n  sequence: []\nvalid_script:\n  sequence: []\n"
    )
    entities = extractor.get_config_defined_entities()
...
def test_extracts_zone_from_config_yaml(tmp_path, extractor):
    (tmp_path / "configuration.yaml").write_text("zone:\n  - name: Work\n")
    entities = extractor.get_config_defined_entities()
    assert "zone.work" in entities


def test_extracts_zone_from_storage(tmp_path, extractor):
...
def test_handles_missing_configuration_yaml(extractor):
    entities = extractor.get_config_defined_entities()
    assert "sun.sun" in entities


def test_handles_parse_error(tmp_path, extractor):
    (tmp_path / "configuration.yaml").write_text("template: !bad_tag\n")
    entities = extractor.get_config_defined_entities()
    assert isinstance(entities, set)
    assert any(
        "Failed to extract entity definitions" in msg for msg in extractor.warnings
    )


def test_reports_summary(tmp_path, extractor):
    (tmp_path / "configuration.yaml").write_text("group:\n  test_group: {}\n")
    extractor.get_config_defined_entities()
    assert any("Config-defined entities:" in info for info in extractor.info)


def test_caches_result(extractor):
    r1 = extractor.get_config_defined_entities()
    r2 = extractor.get_config_defined_entities()
    assert r1 is r2


def test_restore_state_missing_file(extractor):
    result = extractor.load_restore_state_entities()
    assert result == set()


def test_restore_state_bad_json(tmp_path, extractor):
    (tmp_path / ".storage" / "core.restore_state").write_text("not json")
    result = extractor.load_restore_state_entities()
    assert result == set()
    assert any("Failed to load restore state" in msg for msg in extractor.warnings)


def test_restore_state_various_entries(tmp_path, extractor):
    data = {
        "data": [
            "not_a_dict",
            {"state": "not_a_dict"},
...
def test_restore_state_cache(extractor):
    r1 = extractor.load_restore_state_entities()
    r2 = extractor.load_restore_state_entities()
    assert r1 is r2


def test_shared_warnings_append_to_validator(extractor_factory):
    """Warnings/info lists shared with the validator get populated."""
    warnings: list[str] = []
    extractor = extractor_factory(warnings=warnings)
    (extractor.storage_dir / "core.restore_state").write_text("bad json")
    extractor.load_restore_state_entities()
    shared_before = len(warnings)
    extractor.load_restore_state_entities()
    assert len(warnings) == shared_before  # no new warnings on cache hit


def test_shared_info_append(extractor_factory):
    """Extraction summary info appears in shared list."""
    info: list[str] = []
    extractor = extractor_factory(info=info)
    extractor.get_config_defined_entities()
    assert any("Config-defined entities:" in msg for msg in info)


@pytest.mark.parametrize("domain", ["cover", "fan", "lock", "vacuum", "weather"])
def test_extracts_additional_template_domains(tmp_path, domain, extractor):
...
def test_extracts_entities_from_packages(tmp_path, extractor):
...
def test_extracts_entities_from_homeassistant_packages(tmp_path, extractor):
    (tmp_path / "configuration.yaml").write_text(
        "homeassistant:\n"
        "  packages:\n"
        "    lights:\n"
...
def test_include_outside_config_dir_is_rejected(tmp_path, extractor):
...
def test_extracts_entities_from_included_homeassistant_packages(tmp_path, extractor):
...
def test_resolves_include_dir_list_for_automation(tmp_path, extractor):
...
def test_resolves_include_dir_merge_list_for_automation(tmp_path, extractor):
...
def test_missing_explicit_include_does_not_fall_back_to_sidecar(tmp_path, extractor):
...
def test_missing_explicit_script_include_does_not_fall_back_to_sidecar(
    tmp_path, extractor
):
...
def test_dictionary_entities_extract_valid_script_keys_and_ignore_invalid_keys(
    extractor,
):
    result = extractor._extract_script_entities(
        {
            "script": {
                "valid_script": {"sequence": []},
                "UPPERCASE_SCRIPT": {"sequence": []},
                42: {"sequence": []},
            }
        }
    )
...
def test_non_mapping_explicit_script_include_does_not_fall_back_to_sidecar(
    tmp_path, extractor
):
    (tmp_path / "scripts.yaml").write_text("sidecar:\n  sequence: []\n")

    result = extractor._extract_script_entities({"script": "!include_dir_list scripts"})

    assert result == set()


@pytest.mark.parametrize("payload", ["{}", "[]", "null", '"not a mapping"'])
def test_restore_state_malformed_root_is_reported(extractor_factory, payload):
    warnings: list[str] = []
    extractor = extractor_factory(warnings=warnings)
    (extractor.storage_dir / "core.restore_state").write_text(payload)

    assert extractor.load_restore_state_entities() == set()
    assert any("Failed to load restore state" in warning for warning in warnings)


def test_restore_state_data_mapping_keeps_restore_specific_diagnostic(
    extractor_factory,
):
    warnings: list[str] = []
    extractor = extractor_factory(warnings=warnings)
    (extractor.storage_dir / "core.restore_state").write_text(json.dumps({"data": {}}))

    assert extractor.load_restore_state_entities() == set()
    assert any("restore state 'data' must be a list" in warning for warning in warnings)


@pytest.mark.parametrize("payload", ["{}", "[]", "null", '"not a mapping"'])
def test_zone_storage_malformed_root_is_reported(extractor_factory, payload):
    warnings: list[str] = []
    extractor = extractor_factory(warnings=warnings)
    (extractor.storage_dir / "core.zone").write_text(payload)

    assert extractor._extract_zone_entities(None) == set()
    assert any(
        "Failed to extract entity definitions" in warning for warning in warnings
    )


def test_zone_storage_data_list_keeps_zone_specific_diagnostic(extractor_factory):
    warnings: list[str] = []
    extractor = extractor_factory(warnings=warnings)
    (extractor.storage_dir / "core.zone").write_text(json.dumps({"data": []}))
